...
public class CodeScannerActivity extends AppCompatActivity {
    ...
    private FoodTrackerNote selectedFoodTrackerNote;
    private Button deleteButton;

    private FoodTrackerNotificationService service;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        service = new FoodTrackerNotificationService(getApplicationContext());

        deleteButton = findViewById(R.id.deleteInputButton);
        foodNameEditText = (EditText) findViewById(R.id.foodNameEditText);
        expiryDateText = (EditText) findViewById(R.id.expiryDateEditText);
    ...
    private void checkForEditNote() {
        ...
        int passedNoteID = previousIntent.getIntExtra(FoodTrackerNote.NOTE_EDIT_EXTRA, -1);
        selectedFoodTrackerNote = FoodTrackerNote.getNoteForID(passedNoteID);
        if (selectedFoodTrackerNote != null) {
            foodNameEditText.setText(selectedFoodTrackerNote.getFoodName());
            expiryDateText.setText(selectedFoodTrackerNote.getExpiryDate());
        } else {
            deleteButton.setVisibility(View.INVISIBLE);
        }
    }
    public void saveInput(View view){
        FoodTrackSQLiteManager sqLiteManager = FoodTrackSQLiteManager.instanceOfDatabase(this);
        String foodName = String.valueOf(foodNameEditText.getText());
        String expiryDate = String.valueOf(expiryDateText.getText());

        if(selectedFoodTrackerNote == null){
            int id = FoodTrackerNote.foodTrackerNoteArrayList.size();
            FoodTrackerNote newNote = new FoodTrackerNote(id,foodName,expiryDate);
            FoodTrackerNote.foodTrackerNoteArrayList.add(newNote);
            sqLiteManager.addFoodTrackerNoteToDatabase(newNote);
        }else{
            selectedFoodTrackerNote.setFoodName(foodName);
            selectedFoodTrackerNote.setExpiryDate(expiryDate);
            sqLiteManager.updateFoodTrackerNoteListArray(selectedFoodTrackerNote);
        }
        finish();
    }

    public void deleteInput(View view){
        selectedFoodTrackerNote.setDeleted(new Date());
        FoodTrackSQLiteManager sqLiteManager = FoodTrackSQLiteManager.instanceOfDatabase(this);
        sqLiteManager.updateFoodTrackerNoteListArray(selectedFoodTrackerNote);

        service.cancelAlarmIfExists(selectedFoodTrackerNote);
